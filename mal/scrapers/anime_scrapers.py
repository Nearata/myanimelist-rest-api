...
class AnimeScrapers:
    def __init__(self, session: Session, mal_id: Optional[int] = None) -> None:
    ...
    def clubs(self) -> dict:
        soup = self.soup_util.get_soup(f"{self.base_url}/anime/{self.mal_id}/_/clubs")
        clubs = Clubs(soup, self.base_url)
        return clubs()

    def details(self) -> dict:
        soup = self.soup_util.get_soup(f"{self.base_url}/anime/{self.mal_id}")
    ...
    def news(self) -> dict:
        soup = self.soup_util.get_soup(f"{self.base_url}/anime/{self.mal_id}/_/news")
        news = News(soup, self.base_url)
        return news()

    def pictures(self) -> dict:
        soup = self.soup_util.get_soup(f"{self.base_url}/anime/{self.mal_id}/_/pics")
    ...
    def stats(self) -> dict:
        soup = self.soup_util.get_soup(f"{self.base_url}/anime/{self.mal_id}/_/stats")
        stats = Stats(soup)
        return stats()
